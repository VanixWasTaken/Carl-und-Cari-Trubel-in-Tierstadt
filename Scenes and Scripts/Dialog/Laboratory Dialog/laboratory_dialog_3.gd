...
func _ready():
	...
	if Global.talked_to_chameleon_2 == false:
		Global.laboratory_help_button_state = 3
...
##################################  WRITE DIALOG HERE  ###################################
	...
	if dialogs == 1:
		...
		var short_node_rect = short_node.get_child(1)
		var short_node_name = short_node.get_child(3).get_child(0)
		short_node_text.text = "D-D-Das habt ihr sehr gut gemacht. Vielen Dank."
		...
		var short_node_rect = short_node.get_child(1)
		var short_node_name = short_node.get_child(3).get_child(0)
		short_node_text.text = "Diese müsst ihr dann zu der Laborabzugshaube bringen, welche in der linken Wand eingebuchtet ist."
		short_node_rect.texture = PRESET2
		short_node_name.text = "Christina"
