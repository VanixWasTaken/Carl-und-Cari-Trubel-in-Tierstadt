...
##################################  WRITE DIALOG HERE  ###################################
	...
	if dialogs == 1:
		...
		short_node_text.text = "Jetzt m-m-musst du in jedes Reagenzglas Wasser und eine b-b-bestimmte Menge Pulver geben."
		...
		var short_node_rect = short_node.get_child(1)
		var short_node_name = short_node.get_child(3).get_child(0)
		short_node_text.text = "Im linken Glas müssen 0,2 - 0,45 Gramm des lilanen P-P-Pulvers gefüllt werden. Das Pulver kannst du mit d-d-der Waage abwiegen."
		short_node_rect.texture = PRESET3
		short_node_name.text = "Christina"
		dialogs += 1
		same_speaker = false
