...
var Samuel = preload("res://Assets/Art/Characters/Band Members/Saxophonist Sven/sven_icon.png")
##########################################################################################
...
##################################  WRITE DIALOG HERE  ###################################
	...
	if dialogs == 2:
		...
		var short_node_rect = short_node.get_child(1)
		var short_node_name = short_node.get_child(3).get_child(0)
		short_node_text.text = "Was soll das heißen, wer bist du überhaupt?"
		...
		var short_node_rect = short_node.get_child(1)
		var short_node_name = short_node.get_child(3).get_child(0)
		short_node_text.text = "Was war das denn? Am besten reden wir nochmal mit Samuel am Marktplatz."
		short_node_rect.texture = PRESET1
...
"
		...
		var short_node_rect = short_node.get_child(1)
		var short_node_name = short_node.get_child(3).get_child(0)
		short_node_text.text = "Ich bin…, nein WAR der SAXOPHONIST SVEN von BABOON 5! Sagt ihm außerdem noch, dass ich NIE WIEDER zurückkommen werde! *Er haut ab*"
		short_node_rect.texture = PRESET3
		short_node_name.text = "Sven"
		dialogs += 1
